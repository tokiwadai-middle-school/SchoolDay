...
func FindUser(ctx context.Context, exec boil.ContextExecutor, discordId string, selectCols ...string) (*User, error) {
	userObj := &User{}
	...
	query := fmt.Sprintf(
		"select %s from `user` where `discordId`=?", sel,
	)

	q := queries.Raw(query, discordId)
...
func (o *User) Delete(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM `user` WHERE `discordId`=?"
...
func (o *User) Reload(ctx context.Context, exec boil.ContextExecutor) error {
	ret, err := FindUser(ctx, exec, o.DiscordId)
	if err != nil {
		return err
	}
...
func UserExists(ctx context.Context, exec boil.ContextExecutor, discordId string) (bool, error) {
	var exists bool
	sql := "select exists(select 1 from `user` where `discordId`=? limit 1)"

	if boil.IsDebug(ctx) {
		writer := boil.DebugWriterFrom(ctx)
		fmt.Fprintln(writer, sql)
		fmt.Fprintln(writer, discordId)
	}
	row := exec.QueryRowContext(ctx, sql, discordId)
